...
public class HoneywellScannerPlugin extends CordovaPlugin implements BarcodeReader.BarcodeListener {
    ...
    //     List<String> sProfiles = barcodeReader.getProfileNames();
    //     return sProfiles;
    // }
    ...
    throws JSONException {
        if (action.equals("softwareTriggerStart")) {
            ...
            if (barcodeReader != null) {
                try {
                    this.callbackContext = callbackContext;
                    PluginResult pluginResult = new PluginResult(PluginResult.Status.OK, barcodeReader.getProfileNames());
                    pluginResult.setKeepCallback(true);
                    this.callbackContext.sendPluginResult(pluginResult);

                } catch (Exception ex) {
                    e.printStackTrace();
                    NotifyError("getProfile");
                }
            }
        }
        return true;
